...
// CODE HERE
const postTaxPrices = prices.map(x => x * 1.07)
...
const orderTotals = orders.map((element)=>{
...
const bobsPurchases = purchases.filter((element) => element.owner === 'Bob').map((element)=> element.price).reduce((occ, cur) => occ + cur)
